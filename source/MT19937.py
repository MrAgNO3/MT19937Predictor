...
class MT19937():
    ...
    def __init__(self, state = None, ind = 0, state_from_solved_eqns = None, state_from_data = None, useMyMat = 0):
        ...
        if state_from_solved_eqns:
            ...
            self.clone_state_from_data(data, nbits, useMyMat)
        else:
            self.state = state
    ...
    def clone_state_from_data(self, data, nbits, useMyMat=0):
        ...
        # supported_nbits = [1,2,4, 8, 12, 16, 20, 24, 28, 32]
...
class MT19937_symbolic():
    ...
    def _apply_mat(A, mat):
        ...
        job = lambda row:reduce(lambda a,b:a^b,[A[i] for i in row])
